...
TEST_CASE ("as_string") {
  ...
  string_eq (as_string (dirs), string ("Data") * string(URL_SEPARATOR) * "Kernel" *
                                   string(URL_SEPARATOR) * "Plugins");
  ...
  SUBCASE ("tree with empty string") {
    ...
    string_eq (as_string (url ("a") * url ("")), string ("a") * string(URL_CONCATER));
  }
